...
# 第 4 ラウンド:FastApply-7B をそのまま rewrite に使う(2026-08-20 夜)

「FastApply-7B は Qwen2.5-Coder-7B-Instruct ベースだろう。普通の instruct モデルとして扱って書き直させればどうなるか」の実測。同一 20 ブロック・同一 NL 指示・同一 rewrite プロンプト。

## 結果(同一 20 件)

| | EXACT | WS 以上 | sim≥0.95 | avgSim | 中央値レイテンシ |
|---|---|---|---|---|---|
| B ブロック+コード(FA、merge 用途) | 9/20 (45%) | 15/20 (75%) | 17/20 (85%) | 0.975 | 18.2s |
| E1 Agents-A1-4B-Instruct | 2/20 (10%) | 3/20 (15%) | 8/20 (40%) | 0.881 | ~17s |
| **E4 FastApply-7B(rewrite 用途)** | **0/20 (0%)** | **1/20 (5%)** | **5/20 (25%)** | **0.838** | 18.3s |

- E4 の結果ディレクトリ: `results/2026-08-20T12-02-12-036Z/`
- **instruct として振る舞う**: fast-apply タグ(`<updated_code>`)や省略記号の漏れは 0 件。純粋なブロック出力で返す
- **EXACT 0 の主因は微小な空白アーティファクト**: case 7(WS_FILE)は**行末スペースの継承**だけが差(入力行の `false; ` の末尾スペースを新行にもそのまま書いていた)。機能は完全に正しい
- **1 件の截断失敗**: case 5(305 行 Rust)で**出力 14 行で停止**(sim 0.094)。大ブロックの途中離脱
- 高 sim クラスタ: case 6(0.977)/15(0.979)/17(0.993)/10(0.961)/8(0.956)

## 示唆

- **同じモデルでも「用途との適合」が決める**: FastApply-7B は自らの訓練タスク(snippet merge、B: EXACT 45% / avgSim 0.975)では最強だが、**rewrite(ブロック全文書き直し)では 4B-Instruct(0.881)より下(0.838)**。fine-tuning が rewrite 能力を底上げしなかった(訓練分布が snippet-merge だったため)
- 空白の忠実な継承(行末スペースを写す癖)は byte-exact では不利に働く。rewrite 評価の大半の「ほぼ正解」は空白差由来であり、**ws 一致以上の指標が実用上の成功に近い**ことを改めて示す
- 設計結論は強化される: **FastApply-7B は merger として使い続ける**(snippet 経路)、直接書き直しは別の instruct モデル(E1 系)に任せる — あるいは agent が snippet を書く構成(B の agent 版)へ
