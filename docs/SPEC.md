...
Provide a pi.dev-compliant package (Tool/Skill) that merges "partial update snippets" generated by an AI agent (frontier model) into the original source code at high speed and low cost, producing complete updated code, and **writes the merged result directly to a target file using hash-verified scope matching**. The goal is to avoid regenerating entire files, thereby reducing token consumption and maximizing response speed for agents.
...
| `anchor` | String | **Required** | **Exact text in the file for scope matching and hash verification. Must appear exactly once in the file.** |
